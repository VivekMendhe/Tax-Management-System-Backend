...
public class TaxSlabServiceImpl implements TaxSlabService {
	...
	/*
	 * @Override public TaxCalculationResponse calculateTax(TaxCalculationRequest
	 * request) { double grossAmount = request.getAmount(); String regime =
	 * request.getRegime();
	 * 
	 * // Calculate initial tax amount without any deductions double taxAmount = 0;
	 * if ("old".equalsIgnoreCase(regime)) { taxAmount =
	 * calculateOldTax(grossAmount); } else if ("new".equalsIgnoreCase(regime)) {
	 * taxAmount = calculateNewTax(grossAmount); }
	 * 
	 * double cessAmount = taxAmount * CESS_RATE; double totalTaxAmount = taxAmount
	 * + cessAmount;
	 * 
	 * // Check if total tax amount exceeds 50,000 if (totalTaxAmount > 50000) { //
	 * Recalculate taxable income by reducing the standard deduction double
	 * taxableIncome = grossAmount - STANDARD_DEDUCTION; if
	 * ("old".equalsIgnoreCase(regime)) { taxAmount =
	 * calculateOldTax(taxableIncome); } else if ("new".equalsIgnoreCase(regime)) {
	 * taxAmount = calculateNewTax(taxableIncome); } cessAmount = taxAmount *
	 * CESS_RATE; totalTaxAmount = taxAmount + cessAmount; } else { // Set tax and
	 * cess to 0 if total tax amount is less than or equal to 50,000 totalTaxAmount
	 * = 0; cessAmount = 0; }
	 * 
	 * TaxCalculationResponse response = new TaxCalculationResponse();
	 * response.setTaxAmount(taxAmount); response.setCessAmount(cessAmount);
